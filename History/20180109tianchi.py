...
for i in range(90):
    ...
    windGra = df_block["wind"].values.reshape(xsize * ysize, 10)
    windGra = np.median(windGra, axis = 1).reshape(xsize, ysize)
    ...
    plt.savefig('Output1/' + str(i // 18 + 1) + ' Day ' + str(i % 18 + 1) + ' Hour.png')
    del CS
    del CSF
    plt.clf()